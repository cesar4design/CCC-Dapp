...
const mintClick = async (
    ...
    try {
        ...
        updateLoadingText(`Please wait`, guardList, guardToUse.label, setGuardList);
...
const Timer = ({ solanaTime, toTime, setCheckEligibility }: { solanaTime: bigint, toTime: bigint, setCheckEligibility: Dispatch<SetStateAction<boolean>> }) => {
    ...
    if (days > BigInt(0)) {
        return <Text  fontWeight="bold">{days.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}d {hours.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}h {minutes.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}m {seconds.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}s</Text>;
    }
    if (hours > BigInt(0)) {
        return <Text fontWeight="bold">{hours.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}h {minutes.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}m {seconds.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}s</Text>;
    }
    if (minutes > BigInt(0) || seconds > BigInt(0)) {
        return <Text  fontWeight="bold">{minutes.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}m {seconds.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })}s</Text>;
    }
    if (remainingTime === BigInt(0)) {
        setCheckEligibility(true);
    }
    return <Text className={styles.h3} ></Text>;
}
...
export function ButtonList({
    ...
    const listItems = buttonGuardList.map((buttonGuard, index) => (
        <Box minWidth={"420px"} maxWidth={"500px"} margin={"0 auto"}  padding={"15px"} borderRadius={"12px"} color={"white"} background={"#000000"} key={index} marginTop={"10px"}>
            
            <HStack justifyContent={"space-between"}>
                <Text fontSize={"14px"}  >
                    {buttonGuard.header}
                </Text>
                <Flex fontSize={"10px"} marginRight={"0%"}>
                    {
                        ...
                        <><Text className={styles.h3} marginRight={"2"} >End in: </Text><Timer toTime={buttonGuard.endTime} solanaTime={solanaTime} setCheckEligibility={setCheckEligibility} /></>
                    }
                    {
                        ...
                        <><Text className={styles.h3} marginRight={"2"} >Start in: </Text><Timer  toTime={buttonGuard.startTime} solanaTime={solanaTime} setCheckEligibility={setCheckEligibility} /></>
                    }
                </Flex>
            </HStack>
            <SimpleGrid marginTop={"5px"}  columns={2} spacing={5}>
                <Text pt='2' marginTop={"10px"} fontSize={"10px"}>
                    {buttonGuard.mintText}
                </Text>
                <VStack  display={"flex"} marginTop={"5px"} >
                    {process.env.NEXT_PUBLIC_MULTIMINT ?
                        <NumberInput color={"white"} width={"80px"} textAlign={"center"}  value={numberInputValues[buttonGuard.label] || 1} min={1} max={3} size="sm"  onChange={(valueAsString, valueAsNumber) => handleNumberInputChange(buttonGuard.label, valueAsNumber)}>
                            <NumberInputField />
                            <NumberInputStepper>
